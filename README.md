...
- 💼 **雙軌計費模式（Dual Billing Engine）**  
  - **計時發薪（Hourly）**：依實際投入工時自動換算累計產值（預設 $0/h），無合約欠款負擔。
...
| :--- | :--- | :--- | :--- |
| **⏱️ 計時發薪 / 月結回報**<br>(`hourly`) | • 按月回報工時之長期兼職/外包<br>• 計時顧問與諮詢服務<br>• 工時不確定性高之委託 | • 自動依投入工時換算**累計產值**：<br>`產值 = 工時 × $0/h`<br>• 請款報表中可隨時自訂費率 | • ⏱️ 累計工時<br>• 💵 累計產值<br>• ⚡ 時薪標準 ($0/h)<br>• *無合約欠款負擔* |
...
| **月度請款時薪** | ⚡ **預設 $0/h + 即時自訂** | 請款報表預設以 $0/hr 計算，可隨時在輸入框自訂任何金額。 |
